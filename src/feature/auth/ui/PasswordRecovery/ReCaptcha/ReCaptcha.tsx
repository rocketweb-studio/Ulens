...
const ReCaptcha = ({ errorMessage, setCaptcha, invisible }: Props) => {
  ...
  const [loader, setLoader] = useState<"checkbox" | "loading" | "complete">(
    "checkbox",
  );
        ...
        <>
          <div className={s.boxModel}>
            <div className={s.wrapper}>
              <p className={s.errorMessage2}>
                {error === "Verification expired. Check the checkbox again." &&
                  error}
              </p>
              <div>
                {loader === "checkbox" && (
                  <div onClick={onSubmitHandler} className={s.checkbox}></div>
                )}
