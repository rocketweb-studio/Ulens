...
function formatDate(dateString:string,isMobile?:boolean):string {
    const date = new Date(dateString);
    const day = +String(date.getDate()).padStart(2, '0')||'00';
    const month = +String(date.getMonth() + 1).padStart(2, '0')||'00'; // месяцы с 0 до 11
    const year = isMobile
        ? String(date.getFullYear()).slice(-2).padStart(2, '0') || '00'
...
export const DatePicker = ({dateString,labelMobile,label,onClick}:Props) => {
    ...
    const date= formatDate(dateString,isMobile);
    return (
        <div className={s.dateContainer} onClick={onClick}>
            <span className={s.span}>{date}</span>
            <Input type={"text"}  label={isMobile && labelMobile ? labelMobile : label}/>
