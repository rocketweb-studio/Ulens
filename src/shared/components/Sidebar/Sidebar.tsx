...
import {IconHome, IconHomeOutline, IconLogOut, IconPerson, IconPlusSquare} from '@rocketweb-studio/ulens-ui-kit';
...
export const Sidebar = () => {
  ...
  const sidebarLinks = [
    ...
    {
      icon: pathname === Path.UserProfile(data?.id) && params.get("action") === "create" ? IconPlusSquare : IconPlusSquareOutline,
      title: 'Create',
      ...
      <div className={s.sidebarWrapper}>
        {sidebarLinks.map(({title, href, isActive, icon: Icon}, i) =>(
              <FlexContainer className={s.linkWrapper} gap={'13px'} key={i}>
                <Link href={href} className={`${s.link} ${isActive ? s.activeLink : ''}`}><Icon
                  className={s.icon}/>{title}</Link>
              </FlexContainer>
            )
        )}
      </div>
